...
To create a new plugin, we recommend using the CLI tool `npx @jspsych/new-plugin`. This automates setup for a new plugin in either JavaScript or TypeScript. For more information about this tool, as well as step-by-step instructions on how to make a contribution to jsPsych, check out jspsych-contrib's [`README`](https://github.com/jspsych/jspsych-contrib?tab=readme-ov-file#guidelines-for-contributions). 
...
* `trial` is an object containing all of the parameters specified in the corresponding [TimelineNode](../../learn/concepts/timeline.md).
...
While the plugin framework allows you to set up any events that you would like to, including normal handling of `keyup` or `keydown` events, the `jsPsych.pluginAPI` module contains the [`getKeyboardResponse` function](reference/core/jspsych-pluginAPI.md#getkeyboardresponse), which implements some additional helpful functionality for key responses in an experiment.

Here's a basic example. See the [`getKeyboardResponse` docs](reference/core/jspsych-pluginAPI.md#getkeyboardresponse) for additional examples.
...
The data recorded will be that `correct` is `true` and that `rt` is `350`. [Additional data for the trial](../../learn/concepts/plugins.md#data-collected-by-all-plugins) will also be collected automatically.
...
Plugins can optionally support [simulation modes](../../learn/guides/simulation.md).
...
5. In `visual` mode, invoke the `trial()` method of the plugin and then use the artificial data to trigger the appropriate events. There are a variety of methods in the [Plugin API module](reference/core/jspsych-pluginAPI.md) to assist with things like simulating key presses and mouse clicks.
...
If you are developing a plugin with the aim of including it in the main jsPsych repository we encourage you to follow the [contribution guidelines](../contributing/contributing.md#contributing-to-the-codebase). 
