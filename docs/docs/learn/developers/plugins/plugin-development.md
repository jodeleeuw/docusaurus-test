...
The data recorded will be that `correct` is `true` and that `rt` is `350`. [Additional data for the trial](learn/researchers/overview/plugins.md#data-collected-by-all-plugins) will also be collected automatically.
...
Plugins can optionally support [simulation modes](learn/researchers/overview/simulation.md).
